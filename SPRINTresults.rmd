...
subHRinter <- function(dt,x,invar,inter) { # x: event name. Will add "EVENT_" for event coding and "T_" for time coding; covars: covariates in survival model. Put the one of interest (e.g. SBP) as the first one; inter: subgroup membership for interaction analysis
  if (grepl("SAE|ERS|MCE|OHO",x)){
    event <- paste0(x,"_EVNT")
    time <- paste0(x,"_DAYS")
  } else {
  ...
  res <- summary(coxph(form,data=dt))
  pvals <- unname(fit$coefficients[,5])
  hpvals <- p.adjust(pvals,method="hommel")
  return(hpvals[length(hpvals)])
}
```
...
The effects of >140 mmHg on the rate of the primary outcome and on the rate of all-cause death were consistent across the prespecified subgroups, which were significantly higher than the effects of 115-140 mmHg.
...
tb <- subctHR_death3
row.names(tb) <- NULL
colnames(tb) <- c("subgroup","115-140 mmHg","<115 mmHg",">140 mmHg","<115 mmHg HR (95%CI)",">140 mmHg HR (95%CI)")
pander(tb[,c(1:4)],caption="Number of events in each subgroup")
pander(tb[,c(1,5,6)],caption="Proportional HR in each subgroup")
```
